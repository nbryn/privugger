...
class PyMCModelBuilder:
    ...
    def __handle_assign(self, node: model.Assign, condition=None):
        ...
        if node.name in self.pymc_model.named_vars:
    ...
    def __handle_if(self, node: model.If):
        ...
        for child_node in node.body:
            ...
            if child_node.name in self.program_variables:
                self.__map_to_pycm_var(child_node, condition)
            
            # Variable declared inside if
            # For assign: Need default value                
            else:
                (default_value, size) = self.__get_default_value(child_node.value)
                value = self.__map_to_pycm_var(child_node)
                variable = pm_math.switch(condition, value, default_value)
                self.program_variables[child_node.name] = (variable, size)
                pm.Deterministic(child_node.name, variable) 
    ...
    def __get_default_value(self, node):
        ...
        if isinstance(node, model.Subscript):
            ...
            upper = self.__map_to_pycm_var(node.value.upper) if node.value.upper else len(operand)
