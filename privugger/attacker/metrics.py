...
class SimulationMetrics:
        ...
        for j in range(shift, shift+executions):
            for p in range(2):
                ...
                for i in range(10):
                    ...
                    I = mutual_info_regression(
                        [[a] for a in alice], output, discrete_features=True)[0]
                    if name[0] in Is:
                        Is[name[0]].append(I)
                    else:
                ...
                ax[j-shift][p].set_title(
                    f"Parameter {p} opposite was {self.traces[start+stop//2][2][1]}")
    ...
    def plot_mutual_information(self, figsize=(16, 8), as_bar=True):
        ...
        ylim = round(max(self.highest_leakage(head=1, verbose=0),
                     key=lambda x: x[0])[0][0]+0.5)
    ...
    def highest_leakage(self, head=1, verbose=1):
        ...
        for parameter_pos, l in enumerate(self.I):
            ...
            best_dist = list(
                sorted(best_dist, key=lambda x: x[0], reverse=True))
            if verbose:
                print(
                    f"The distribution that had the most leakage was {best_dist[:head]} for parameter {parameter_pos}")
            ...
            for trace, names, info in self.traces:
                ...
                try:
                    ...
                    if pos < 10:
                        ...
                        continue
                I_ao = mutual_info_regression(
                    [[j] for j in alice], output, discrete_features=discrete)[0]
                while (len(info) == 1):
                    # Used to unwrap the inner information in case of subtypes such as List[List[Tuple[...]]]
                    info = info[0]
