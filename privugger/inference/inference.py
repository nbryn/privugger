...
def concatenate(distribution_a, distribution_b, type_of_dist, axis=0):
    ...
    with global_model as model:
        val = pm.math.concatenate(
            (
                distribution_a.pymc3_dist(distribution_a.name, []),
                distribution_b.pymc3_dist(distribution_b.name, []),
            ),
            axis=axis,
        )
    ...
    if method == Method.PYMC:
        if program is not None:
            ...
            if use_new_method:
                ...
                with model:
                    ...
                    WhiteBoxAstTransformer(
                        ...
                        model,
                        method,
                    ).transform(abstract_syntax_tree, function_def)
            ...
            with model:
                ...
                print("SAMPLING")
                trace = pm.sample(
                    draws=draws, chains=chains, cores=cores, return_inferencedata=True
                )
                global_priors = []
