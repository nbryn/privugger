...
class Program:
    ...
    def add_observation(self, constraints, precision=0.01):
        ...
        if val2 == None and cons2 != "":
            raise ValueError(
                "Malformed observation: please review the observation to ensure that the syntax is correct.")
        ...
        def partial1(x): return None
        def partial2(x): return None
        if name in var_names or "output" in name:
            ...
            raise ValueError(
                "Observation was not known. Make sure that the name is part of the names in privugger.Datastructure")
    ...
    def _unwrap_constrain(self, value, cons, precision, i=0):
        ...
        def inner(distribution):
            if cons == ">":
                ...
                raise ValueError(
                    f"The program does not support {cons} as a constrain")
        return inner
