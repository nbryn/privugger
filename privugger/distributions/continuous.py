...
class Uniform(Continuous):
    ...
    def scipy_dist(self, name):
        dist = (lambda siz: st.uniform(self.lower, self.upper-self.lower).rvs(siz)) if self.num_elements == - \
            1 else (lambda siz: st.uniform(self.lower, self.upper-self.lower).rvs((self.num_elements, siz)))
        return name, dist
...
class Normal(Continuous):
    ...
    def scipy_dist(self, name):
        dist = (lambda siz: st.norm(self.mu, self.std).rvs(siz)) if self.num_elements == - \
            1 else (lambda siz: st.norm(self.mu, self.std).rvs((self.num_elements, siz)))
        return name, dist
...
class Exponential(Continuous):
    ...
    def scipy_dist(self, name):
        dist = (lambda siz: st.expon(self.lam).rvs(siz)) if self.num_elements == - \
            1 else (lambda siz: st.expon(self.lam).rvs((self.num_elements, siz)))
        return name, dist
...
class Beta(Continuous):
    ...
    def scipy_dist(self, name):
        dist = (lambda siz: st.beta(self.alpha, self.beta).rvs(siz)) if self.num_elements == - \
            1 else (lambda siz: st.beta(self.alpha, self.beta).rvs((self.num_elements, siz)))
        return name, dist
